...
class storesController {
  ...
  static async getStores(
    ...
    const { stores, total } = await StoreServices.getAllStores(value, where, include)
  ...
  // get a single shop
  static async singleStore(req: RequestWithUser, res: Response): Promise<Response> {
  ...
  // update store
  static async updateStore(req: RequestWithUser, res: Response): Promise<Response> {
    const { id: userId, role: userRole } = req.user!

    // Validate the param id
    const { error: paramIdError, value: paramValue } = validateUUIDV4(req.params.id)
    if (paramIdError) {
      return res.status(400).json({
        status: 'fail',
        message: paramIdError.message,
      })
    }
    const { id } = paramValue
